...
read(50,*)
write(flnm3,"('output/',A,'/flux3d_JUP',I0,'keVJG-eq.dat')") SZA, IonEnergy
write(flnm2,"('output/',A,'/airglow_JUP',I0,'keVJG-eq.dat')") SZA, IonEnergy
read(50,9999)! flnm3 !output/SE/flux3d_JUP[Energy]keVJG.dat
read(50,9999)! flnm2 !output/SE/airglow_JUP[Energy]keVJG.dat
do i = 1,2
	read(50,*) !skip some lines
end do
write(flnm4,"('../Output/',I0,'/H2*_Prod-Comb.dat')") IonEnergy
read(50,9999)! flnm4 !../Output/[Energy]keV/H2_Excite_Prod_Comb.dat
write(*,*) flnm4
write(flnm,"('../Output/',I0,'/H2+_Prod-Comb.dat')") IonEnergy
open(unit=27,file=flnm,status='old')
write(*,*) flnm
write(flnm,"('../Output/',I0,'/H+_Prod-Comb.dat')") IonEnergy
...
write(flnm,"('common/output/',A,'/secprd',I0,'-eq.dat')") SZA, IonEnergy
open(22,FILE=flnm)
write(flnm,"('common/output/',A,'/escapeflux',I0,'-eq.dat')") SZA, IonEnergy
open(24,file=flnm)
write(flnm,"('common/output/',A,'/escapecurrent',I0,'-eq.dat')") SZA, IonEnergy
open(25,file=flnm)
...
do i=nz,1,-1
  ...
  read(27,*) dummy1(i), H2p(i)
  read(28,*) dummy1(i), Hp(i)
end do
...
open(unit=27,file='common/output/SE/AirglowUVLaTeX-eq.dat',access='append',action='write')
open(unit=28,file='common/output/SE/AirglowIonsLaTeX-eq.dat',access='append',action='write')
